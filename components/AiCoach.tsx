...
const AiCoach: React.FC<AiCoachProps> = ({ data }) => {
  ...
  const handleGetAdvice = async () => {
    if (!data) return;
    setLoading(true);
    const result = await getCoachingAdvice(data);
    setAdvice(result);
    setLoading(false);
    setHasFetched(true);
  };
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">AI Coach Analysis</h2>
              <p className="text-indigo-200 text-sm">Powered by Gemini 3 Flash</p>
            </div>
          </div>
          {!hasFetched && !loading && (
            <button
