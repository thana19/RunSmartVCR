...
const Results: React.FC<ResultsProps> = ({ data }) => {
  ...
  const getZoneIcon = (name: string) => {
    // We check against the English keys or structure, but here we simply map based on index or checks. 
    // However, since 'name' comes from App.tsx which uses translation, we should check broadly or rely on order.
    // For simplicity, let's rely on the text content matching one of our known zones or default.
    // A better approach is passing an ID, but let's check basic keywords that exist in both langs or just default icons.
    if (name.includes('Easy') || name.includes('Recovery')) return <Wind className="w-5 h-5" />;
    if (name.includes('Marathon')) return <Activity className="w-5 h-5" />;
    if (name.includes('Threshold')) return <TrendingUp className="w-5 h-5" />;
    if (name.includes('Interval')) return <Flame className="w-5 h-5" />;
    if (name.includes('Repetition')) return <Zap className="w-5 h-5" />;
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-lg flex flex-col items-center justify-center text-center">
            <h3 className="text-slate-400 text-sm font-medium uppercase tracking-wider mb-1">{t.avgPace}</h3>
        ...
        <div className="bg-slate-800 p-5 rounded-2xl border border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.1)] flex flex-col items-center justify-center text-center relative overflow-hidden">
            ...
            <h3 className="text-emerald-400 text-sm font-bold uppercase tracking-wider mb-1 relative z-10">{t.thresholdPace}</h3>
        ...
        <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-lg flex flex-col items-center justify-center text-center">
            <h3 className="text-slate-400 text-sm font-medium uppercase tracking-wider mb-1">{t.vo2Max}</h3>
      ...
      <div className="bg-slate-800 rounded-2xl shadow-lg border border-slate-700 overflow-hidden">
        <div className="p-4 bg-slate-900/50 border-b border-slate-700 flex items-center gap-2">
          ...
          <h3 className="font-bold text-white">{t.zonesTitle}</h3>
        </div>
